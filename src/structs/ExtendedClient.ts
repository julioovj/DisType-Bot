...
export class ExtendedClient extends Client {
  ...
  private registerCommands(commands: Array<ApplicationCommandDataResolvable>) {
    this.application?.commands
      .set(commands)
      .then(() => {
        console.log("✅ Comandos (/) foram definidos!".green);
      })
      .catch((error) => {
        console.log(`❌ Erro ao obter a lista de comandos!\n${error}`.red);
      });
  }
  private registerModules() {
    const slashComands: Array<ApplicationCommandDataResolvable> = new Array();
    ...
    this.on("ready", () => this.registerCommands(slashComands));
  }
